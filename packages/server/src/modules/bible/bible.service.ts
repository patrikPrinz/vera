import { ConflictError, NotFoundError } from "../../shared/error_handler/errors.js";
import BibleRepository from "./bible.repository.js";
import { BibleBook, BibleChapter, BibleTranslation, BibleTranslationMetadata, BibleVerse } from "./bible.types.js";
...
export class BibleService {
  ...
  getMetadataService = async (translation: string): Promise<BibleTranslationMetadata> => {
  ...
  getChaptersService = async (translation: string, book: number): Promise<BibleChapter[]> => {
    const data = await this.repository.getChapters(translation, book);
    return data
  }

  getVersesService = async (translation: string, book: number, chapter: number): Promise<BibleVerse[]> => {
    const data = await this.repository.getVerses(
      translation,
      book,
      chapter
    );
